...
uses
  System.Classes, Vcl.Dialogs, SysUtils, System.JSON, Vcl.Grids, System.IOUtils,
  printer;

type
  // Class responsible for exporting the shopping cart data into a JSON file (receipt)
  TJsonPrinter = class(TInterfacedObject, IPrinter)
  private
...
constructor TJsonPrinter.Create(const ACartGrid: TStringGrid;
...
begin
  ...
  try
    ...
    // Prepare the SaveDialog
    openSaveDialog.Title := 'Save Receipt As';
    openSaveDialog.Filter := 'JSON Files (*.json)|*.json';
    openSaveDialog.InitialDir := FReceiptsFolder;

    openSaveDialog.fileName := 'Receipt_' +
...
function TJsonPrinter.AddProducts: TJSONArray;
var
  products: TJSONArray;
  i: Integer;
begin
...
function TJsonPrinter.AddProduct(name: string; price: string; quantity: Integer;
totalProdPrice: string): TJSONOBject;
var
  product: TJSONOBject;
begin
